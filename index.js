...
document.write(
  `<header><a href='#'><img src=${pathPictures.logo} alt='logo'/></a></header>`
);
document.write(`<h1>Welcome Back, ${userData.nameUser}!</h1>`);
document.write(`<h2>My Cards</h2>`);
document.write(`<ul class='content'>`);
for (let numCard = 0; numCard < userData.userCards.length; numCard++) {
  ...
  document.write(`<img src=${
    card.networkType === "Mastercard" ? pathPictures.master : pathPictures.visa
  } 
  ...
  document.write(`<h4>History Transactions</h4>`);
  document.write(`<ul class='transaction-list'>`);
  for (
    let numTransaction = 0;
    numTransaction < card.historyTransactions.length;
    numTransaction++
  ) {
    let transaction = card.historyTransactions[numTransaction];

    document.write(`<li class='transaction'>`);
    let iconPositiveDebiting =
      transaction.amount > 0
        ? pathPictures.balance.positive
        : pathPictures.balance.debiting;
    document.write(
      `<div class='transaction__icon'><img src=${iconPositiveDebiting} alt='icon'/></div>`
    );
